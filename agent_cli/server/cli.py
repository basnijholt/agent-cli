...
def install_cmd(  # noqa: PLR0912, PLR0915
    ...
    **Examples:**
        ...
        agent-cli server install whisper tts

        # Install all services
        agent-cli server install --all

        # Skip confirmation prompts
        agent-cli server install whisper -y
...
def uninstall_cmd(
    ...
    **Examples:**
        ...
        agent-cli server uninstall whisper tts

        # Uninstall all services
        agent-cli server uninstall --all
...
def start_cmd(
    ...
    """Start all services in a Zellij session.
    ...
    **Examples:**
    ...
    Start services and attach:
        `agent-cli server start`

    Start in background (for scripts or automation):
        `agent-cli server start --no-attach`
    ...
    from agent_cli.core.utils import print_with_style  # noqa: PLC0415
    from agent_cli.install.common import get_script_path  # noqa: PLC0415

    try:
        script_path = get_script_path("start-all-services.sh")
    except FileNotFoundError as e:
        err_console.print("[bold red]Error:[/bold red] Service scripts not found")
        console.print(str(e))
        raise typer.Exit(1) from None
    ...
    try:
        ...
        err_console.print(
            f"[bold red]Error:[/bold red] Failed to start services. Exit code: {e.returncode}",
        )
