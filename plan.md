...
## Next Up (Immediate Wins, mem0-inspired) — To Do
- [ ] LLM-driven consolidation pass after retrieval: run a small function-call model over overlapping facts to label ADD/UPDATE/DELETE/KEEP and retire stale/conflicting facts (beyond latest-wins). Inspired by mem0’s add/update/delete flow (`mem0/mem0/configs/prompts.py`, update prompts in `mem0/mem0/memory/main.py`).
