...
Run ASR (Automatic Speech Recognition) servers.
...
| Home Assistant integration | `whisper` (has Wyoming protocol) |
| iOS Shortcuts integration | `transcription-proxy` |
| Forwarding to cloud providers | `transcription-proxy` |
| Privacy-focused (no cloud) | `whisper` |
| VRAM-constrained system | `whisper` (TTL unloading) |
